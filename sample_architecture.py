...
def cnn_model_fn(features, labels, mode):
    ...
    input_layer = tf.reshape(features["x"], [-1, 112, 32, 1])
    ...
    # output has size of [batch_size, 112, 32, 32]

    pool1 = tf.layers.max_pooling2d(inputs=conv1, pool_size=[2, 2], strides=2)
    # output has size of [batch_size, 56, 16, 32]
    ...
    conv2 = tf.layers.conv2d(
        ...
        padding="same",
        activation=tf.nn.relu)
    # output has size of [batch_size, 56, 16, 64]

    pool2 = tf.layers.max_pooling2d(inputs=conv2, pool_size=[2, 2], strides=2)
    # output has size of [batch_size, 28, 8, 64]

    # flatten layer so we can put it into our fully connected layer
    pool2_flat = tf.reshape(pool2, [-1, 28 * 8 * 64])
    # output has shape of [batch_size, 50176]
    ...
    print(labels, logits)
    loss = tf.losses.absolute_difference(labels=tf.reshape(labels, [100, 1]), predictions=tf.reshape(logits, [100, 1]))
    print("LOSS", loss)
