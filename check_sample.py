...
def check_slice_sample(sample_path):
    ...
    rect = patches.Rectangle(corner_a, labelling.shape[0], labelling.shape[1], linewidth=1, edgecolor='r',
                             facecolor='none')

    print(np.unique(labelling))

    padded_labelling[corner_a[0]:corner_b[0], corner_a[1]:corner_b[1]] = labelling

    masked_data = np.ma.masked_where(padded_labelling == 0, padded_labelling)

    fig, ax = plt.subplots(1)

    ax.imshow(sample.T, interpolation="none", origin='lower', cmap='gray', vmin=-2)
