...
train_X = np.zeros([len(slices) * patches_per_image, 320, 40])
train_Y = np.zeros([len(slices) * patches_per_image, 320, 40])

print(len(slices) * patches_per_image)
...
for cur_slice, mask in zip(slices, masks):
    ...
    for _ in range(patches_per_image):
        ...
        train_X[i] = slice_patch
        train_Y[i] = mask_patch
        i += 1
