...
def generate_slice_samples(dataset_dir, sample_dir, X_size=(124, 332), y_size=(36, 84),
                           spacing=(2.0, 2.0, 2.0), no_of_samples=5, no_of_vertebrae_in_each=2,
                           file_ext=".nii.gz"):

    X_size = np.array(X_size)
    y_size = np.array(y_size)
    offset = ((X_size - y_size) / 2.0).astype(int)
    border_paddings = np.array(list(zip(offset, offset))).astype(int)
    ...
    for cnt, data_path in enumerate(paths):
        ...
        for i in cuts:
            ...
            volume_slice = np.pad(volume_slice, border_paddings, mode="constant")
            sample_labels_slice = np.pad(sample_labels_slice, border_paddings, mode="constant")
            ...
            if volume_slice.shape[0] < X_size[0]:
                dif = X_size[0] - volume_slice.shape[0]
            ...
            if volume_slice.shape[1] < X_size[1]:
                dif = X_size[1] - volume_slice.shape[1]
            ...
            j = 0
            while True:
                random_area = volume_slice.shape - X_size
                ...
                cropped_sample_labels_slice = sample_labels_slice[corner_c[0]:corner_d[0], corner_c[1]:corner_d[1]]
